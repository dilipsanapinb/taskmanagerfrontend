...
const TasksTable = ({ tasks, handleDelete }) => {
    ...
    const confirmTaskComplete = async () => {
        try {
            ...
            await axios.patch(`https://taskmanager-900c.onrender.com/task/update/${completed._id}`, updateData, config);
            // console.log(data);
    ...
    useEffect(() => {
        const fetchUserName = async () => {
            try {
                ...
                const [userData, assignedByData] = await Promise.all([await axios.get(`https://taskmanager-900c.onrender.com/user/${task.assignedTo}`, config),
                
                await axios.get(`https://taskmanager-900c.onrender.com/user/${task.assignedBy}`, config)])
